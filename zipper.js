...
function init() {
  ...
  if (autoOpen) {
    autoOpenInterval = self.setInterval(function () {
      autoOpenDraw();
    }, 1000 / autoOpenSpeed);
  }
}
...
function placeToothsDXOnPath(path) {
  ctx.save();
  var Tarr = parameterizeCurveOpt(
    path,
    getArcLength(path) / NumOfTooths,
    PointsPerTooth / 2
  );
...
function newtonSqrt(input) {
  // too slow.. Use Math.sqrt instead
  var x = 1;
  for (var i = 0; i < 10; i++) x = x - (x * x - input) / (2 * x);
  return x;
}
...
function myMove(e) {
  ...
  if (dragok) {
    ...
    if (!autoOpen) {
      if (
        pageX < x + baseWIDTH + baseCanvas.offsetLeft &&
        pageX > x + baseCanvas.offsetLeft &&
        pageY < y + baseHEIGHT + baseCanvas.offsetTop &&
